...
export async function generateMetadata({ params }) {
  ...
  return product ? {
    title: `${product.model} ${product.name} | Synoveta`,
      ...
      <div className='container'>
        ...
        <div className='conference-single-hero-grid'>
          <div className='conference-single-copy'>
            <p className='eyebrow'>{system.shortName}</p>
          ...
          <div className='conference-single-image'>
            <Image src={product.image} alt={`${product.model} ${product.name}`} fill priority sizes='(max-width:760px) calc(100vw - 30px), 50vw'/>
          </div>
        </div>
      </div>
    </section>
