...
export async function generateMetadata({params}){const {subcategory}=await params;const range=gunneboRanges[subcategory];return range?{title:`${range.title} | Synoveta`,description:range.copy}:{};}
...
export default async function GunneboRangePage({params}) {
  ...
  return <><Header activePage="products"/><main className="product-page"><div className="container">
    <Link className="back-link" href="/products/access-control-turnstile/gunnebo/"><ArrowLeft/> Back to Synoveta Turnstiles</Link>
