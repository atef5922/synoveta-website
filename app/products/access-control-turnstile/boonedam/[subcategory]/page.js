...
export async function generateMetadata({params}){const {subcategory}=await params;const range=boonedamRanges[subcategory];return range?{title:`${range.title} | Synoveta`,description:range.copy}:{};}
...
export default async function BoonedamRangePage({params}) {
  ...
  return <><Header activePage="products"/><main className="product-page"><div className="container">
    <Link className="back-link" href="/products/access-control-turnstile/boonedam/"><ArrowLeft/> Back to Synoveta Turnstiles</Link>
    <div className="turnstile-heading"><p className="eyebrow">Synoveta entrance control</p><h1>{range.title}</h1><p className="lead">{range.copy}</p></div>
