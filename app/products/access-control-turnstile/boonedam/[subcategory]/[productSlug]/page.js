...
export async function generateMetadata({params}){const {subcategory,productSlug}=await params;const product=boonedamRanges[subcategory]?.products.find((item)=>item.slug===productSlug);return product?{title:`${product.name} | Synoveta`,description:product.summary}:{};}
