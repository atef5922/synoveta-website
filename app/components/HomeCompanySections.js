  ...
  return <div className="home-company-sections">
    <section className="home-company" aria-labelledby="home-company-title">
      <div className="container">
        <dl className="home-company-stats">{stats.map(([Icon,value,title,copy])=><div key={title}><Icon aria-hidden="true"/><div><dt>{title}</dt><dd>{value}</dd><p>{copy}</p></div></div>)}</dl>
        ...
        <div className="home-company-services">{services.map(([Icon,title,copy,href])=><article key={title}><Icon aria-hidden="true"/><div><h3>{title}</h3><p>{copy}</p><Link href={href} aria-label={`Learn more about ${title}`}>Learn More <ArrowRight aria-hidden="true"/></Link></div></article>)}</div>
      </div>
    </section>
  </div>;
}
